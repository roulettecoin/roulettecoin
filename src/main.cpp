...
#if defined(NDEBUG)
# error "Roulettecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0000039a2d3dae2de2ebd1561d683f7fbafb7b4e3271ec6c0f70e687476779bb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Roulettecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Roulettecoin Signed Message:\n";
...
{
    // Roulettecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Roulettecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Roulettecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000cd1a53066e69fc1899114906aff283eff7505bd3d657a5fea0f1545a95b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1398943479;
        ...
        assert(block.hashMerkleRoot == uint256("0x5620aed9a817ac49c54cbc9240954cb60027242a1bac9dc6b83589466dd244eb"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RoulettecoinMiner : ProcessBlock, block not accepted");
    }
...
void static RoulettecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running RoulettecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RoulettecoinMiner, pwallet));
}
